...
#'@param sudo Logical. If TRUE, run as sudo. Default is FALSE.
#'
#'@return DetDiet_Processed.gz
#'
#'@export

process_det_diet <- function(atl.dir, detDietfile, outputname, sudo =F) {
  
  #zip up file
  if(sudo){
    system2('sudo',args = c('gzip','-k',paste0(atl.dir,detDietfile)))
    ...
    #concatenate them
    system2('sudo',args = c('cat',paste0(atl.dir,'neus_outputDetDietHead.gz'),paste0(atl.dir,'neus_outputDetDiet_nz.gz')),stdout = paste0(atl.dir,outputname))
